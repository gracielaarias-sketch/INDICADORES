...
with st.expander("👉 Ver Resumen"):
    if not df_op_raw.empty:
        df_op_f = df_op_raw[(df_op_raw['Fecha_DT'].dt.normalize() >= ini) & (df_op_raw['Fecha_DT'].dt.normalize() <= fin)]
...
if not df_prod_f.empty:
    ...
    st.plotly_chart(px.bar(df_prod_f.groupby(c_m)[[c_b, c_r]].sum().reset_index(), x=c_m, y=[c_b, c_r], barmode='stack', color_discrete_map={c_b:'#2ecc71',c_r:'#f1c40f'}), use_container_width=True)
    ...
    with st.expander("📂 Tablas Detalladas"):
        ...
        cols_group = [col for col in [c_cod, c_m, 'Fecha_Str'] if col is not None]
        df_tab = df_prod_f.groupby(cols_group)[[c_b, c_r, c_o]].sum().reset_index()
        # FIX VALUEERROR: Matching ascending list length
...
st.plotly_chart(px.pie(df_f_bal, values='Tiempo (Min)', names='Tipo', color='Tipo', color_discrete_map={'Produccion':'#2ecc71','Parada':'#e74c3c'}, hole=0.4), use_container_width=True)
...
def generar_pdf_final(area, fecha_sel):
    ...
    k = get_kpis_safe(df_oee_raw[df_oee_raw['Fecha_Filtro'] == f_dt], area)
    ...
    for m in sorted(df_a['Máquina'].unique()):
        km = get_kpis_safe(df_oee_raw[df_oee_raw['Fecha_Filtro'] == f_dt], m)
    ...
    if not df_fal.empty:
        res = df_fal.groupby(['Máquina', 'Nivel Evento 6', 'Operador'])['Tiempo (Min)'].sum().reset_index().sort_values(['Máquina', 'Tiempo (Min)'], ascending=[True, False]).head(15)
        res.columns = ['Maq', 'Falla', 'Levanto', 'Min']
        draw_table(pdf, res, [40, 70, 50, 30])
...
with c1:
    if st.button("🏗️ Descargar PDF Estampado"):
        ...
        if res: st.download_button(f"Confirmar_Estampado_{dia_pdf}.pdf", res, f"Reporte_Estampado_{dia_pdf}.pdf")
with c2:
    if st.button("🤖 Descargar PDF Soldadura"):
        res = generar_pdf_final("SOLDADURA", dia_pdf)
        if res: st.download_button(f"Confirmar_Soldadura_{dia_pdf}.pdf", res, f"Reporte_Soldadura_{dia_pdf}.pdf")
