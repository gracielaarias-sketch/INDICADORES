    ...
    try:
        ...
        def process_df(url):
            try:
                ...
                cols_num = ['Tiempo (Min)', 'Buenas', 'Retrabajo', 'Observadas', 'Tiempo Producción (Min)', 'Tiempo Ciclo']
                for c in cols_num:
...
# ==========================================
# 4. SECCIÓN OEE (LÓGICA ACTUALIZADA)
# ==========================================
st.title("🏭 INDICADORES FAMMA")

def get_metrics(name_filter):
    m = {'OEE': 0.0, 'DISP': 0.0, 'PERF': 0.0, 'CAL': 0.0}
    d_oee = df_oee_f[df_oee_f.apply(lambda row: row.astype(str).str.upper().str.contains(name_filter.upper()).any(), axis=1)]
    ...
    if not d_oee.empty and d_oee['Fecha_Filtro'].nunique() == 1:
        ...
        if not d_prod.empty:
            ...
            total_esperado = d_prod['esperadas'].sum()
            m['PERF'] = total_real / total_esperado if total_esperado > 0 else 0.0
