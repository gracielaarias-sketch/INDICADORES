...
if not df_prod_f.empty:
    ...
    if c_maq:
        ...
        with st.expander("📂 Tablas Detalladas por Código, Máquina y Fecha"):
            ...
            st.dataframe(df_tab.sort_values(sort_cols if 'sort_cols' in locals() else cols_group, ascending=[True, False]), use_container_width=True, hide_index=True)
...
# ==========================================
# 11. MÓDULO PDF PROFESIONAL (NUEVO)
# ==========================================
st.markdown("---")
...
def generar_pdf_original(area_nombre, fecha_sel):
    ...
    if df_area.empty:
        st.error(f"No hay datos para {area_nombre} el día {fecha_sel}"); return None
    ...
    # 5. DETALLE DE FALLAS (QUIEN LEVANTO)
    ...
    df_p_area = df_prod_dia[df_prod_dia['Máquina'].isin(df_area['Máquina'].unique())]
    if not df_p_area.empty:
        res_prod = df_p_area.groupby('Máquina')[['Buenas', 'Retrabajo', 'Observadas']].sum().reset_index()
